...
void font_tahomaInit() {
	...
	font_config.getChar					= getCharTahoma;
}


Coordinates getCharTahoma(char character) {
	Texture* font_texture;
	if (font_config.weight == FONT_BOLD) {
		font_texture = &tahoma_bold_texture;
		switch (character) {
			case 'A': return (Coordinates){  0, 0, font_texture, 8, 12}; break;
			case 'B': return (Coordinates){  8, 0, font_texture, 7, 12}; break;
			...
			case '2': return (Coordinates){351, 0, font_texture, 7, 12}; break;
			...
			case '.': return (Coordinates){454, 0, font_texture, 2, 12}; break;
